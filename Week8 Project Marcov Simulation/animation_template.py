...
# Visualisation of supermakrt background

class SupermarketMap:

    def __init__(self, layout, tiles):     # layout: a string with each character representing a tile
    ...
    def get_tile(self, char):              # returns the array for a given tile charakter
        if char == '#':
    ...
    def prepare_map(self):                 # prapares the entire image as a big numpy array
    ...
    def draw(self, frame, offset=OFS):     # draws the image into a frame
                                           # offset pixels from the top left corner

        frame[OFS:OFS+self.image.shape[0], OFS:OFS +
              self.image.shape[1]] = self.image

    def write_image(self, filename):       # writes the image into a file
