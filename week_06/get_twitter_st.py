...
elon = client.get_user(username='elonmusk', user_fields=[
                       'name', 'id', 'created_at'])
# print(elon)

print(
    f'the user with name {elon.data.name} and ID {elon.data.id} created its twitter account on {elon.data.created_at}')
...
elon_tweets = client.get_users_tweets(id=elon.data.id, tweet_fields=[
                                      'id', 'text', 'created_at'], max_results=20)
# print(elon_tweets.data)
for tweet in elon_tweets.data:
    print(
        f"the user {elon.data.name} at {tweet.created_at} wrote:{tweet.text}\n")
...
# query = 'from:cher'
query = 'climate+change -is:retweet'


search_tweets = client.search_recent_tweets(
    query=query, tweet_fields=['id', 'created_at', 'text'], max_results=100)
print(search_tweets)
...
# paginator = tweepy.Paginator(client.search_recent_tweets, tweet_fields=[
#                              'id', 'created_at', 'text'], query=query).flatten(limit=200)
# print(paginator)
